...
|------|-------------|------|---------|:--------:|
| <a name="input_project_id"></a> [project\_id](#input\_project\_id) | The GCP project ID (6-30 characters, lowercase alphanumeric and hyphens, must start with a letter) | `string` | n/a | yes |
| <a name="input_project_prefix"></a> [project\_prefix](#input\_project\_prefix) | Prefix for naming resources (e.g., 'ashes-dev'). Must start with a letter and contain only lowercase letters, digits, and hyphens. | `string` | n/a | yes |
...
| <a name="input_explicit_zones"></a> [explicit\_zones](#input\_explicit\_zones) | Explicit list of zones to use for subnet layout (e.g. ["us-central1-a", "us-central1-b", "us-central1-c"]).<br/><br/>⚠️  PRODUCTION REQUIREMENT: always set this variable for any environment with live workloads.<br/>GCP occasionally changes how many zones the data source returns (e.g. 3 → 4 when a new<br/>zone is added to a region). Because subnet CIDRs are derived from zone count, a change will<br/>cause Terraform to plan subnet destruction and recreation — which evicts all running VMs.<br/><br/>When empty, zones are auto-discovered via data.google\_compute\_zones (acceptable for dev/test<br/>environments where occasional disruption is tolerable). | `list(string)` | `[]` | no |
...
| <a name="input_region"></a> [region](#input\_region) | The primary GCP region for resources (e.g., 'us-central1', 'europe-west1') | `string` | `"us-central1"` | no |
