...
- resource.aws_cloudwatch_metric_alarm.usage (modules/aws/governance/service-quotas/main.tf#L30)
- resource.aws_servicequotas_service_quota.this (modules/aws/governance/service-quotas/main.tf#L10)
...
| <a name="input_quota_increases"></a> [quota\_increases](#input\_quota\_increases) | Map of quota-increase requests keyed by an arbitrary name. Each entry files a service-quota request and provisions an AWS/Usage alarm at ~80% of the requested value. | <pre>map(object({<br/>    service_code = string # e.g. "ec2"<br/>    quota_code   = string # e.g. "L-1216C47A"<br/>    value        = number # requested quota value<br/>    # AWS/Usage alarm dimensions (audit A2). REQUIRED for a functional usage<br/>    # alarm — omit and the entry files the quota request but gets no alarm.<br/>    # e.g. { service = "EC2", class = "Standard/OnDemand", type = "Resource", resource = "vCPU" }<br/>    usage_metric = optional(object({<br/>      service  = string<br/>      class    = string<br/>      type     = string<br/>      resource = string<br/>    }))<br/>  }))</pre> | `{}` | no |
