...
| <a name="input_enable_cloudwatch_logs"></a> [enable\_cloudwatch\_logs](#input\_enable\_cloudwatch\_logs) | When true, attach a CloudWatch Logs group to the org trail and create CIS control-plane metric-filter alarms. Default false preserves the deliberate S3-only design. PREVIEW: opt-in (default off), not yet validated against a real apply. | `bool` | `false` | no |
