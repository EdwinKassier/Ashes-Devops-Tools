...
| <a name="input_max_ports_per_vm"></a> [max\_ports\_per\_vm](#input\_max\_ports\_per\_vm) | Maximum number of ports per VM for dynamic port allocation (64–32768). Must be >= min\_ports\_per\_vm. Only meaningful when enable\_dynamic\_port\_allocation is true. | `number` | `null` | no |
...
| <a name="input_router_name"></a> [router\_name](#input\_router\_name) | Name of the Cloud Router. Required if create\_router is false (to reference an existing router). Auto-generated from the NAT name when create\_router is true. | `string` | `""` | no |
