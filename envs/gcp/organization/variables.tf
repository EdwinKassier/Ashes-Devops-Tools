...
variable "vpc_sc_additional_restricted_services" {
  description = "Extra service hostnames to add to the hub data perimeter. Default [] = unchanged. Set the automation-ingress identities below before expanding under enforcement. PREVIEW (not yet validated against a real apply) — validate under vpc_sc_enable_dry_run first."
  type        = list(string)
  default     = []
}

variable "vpc_sc_ingress_identities" {
  description = "Automation identities granted full ingress into the hub data perimeter, e.g. the TFC-run SA. Set BEFORE expanding restricted_services under enforcement. Default [] = no ingress policy (unchanged)."
  type        = list(string)
  default     = []
}
...
variable "logging_project_id" {
  description = "Dedicated logging project ID for the org audit sink/bucket (separation of duties). Default null = admin project (unchanged). PREVIEW: opt-in (default off), not yet validated against a real apply.; ensure CMEK cross-project access."
  type        = string
  default     = null
}
...
variable "enable_privilege_split" {
  description = "Enable the bootstrap SA privilege split: route high-privilege org roles to a separate resman SA. Default false = unchanged. Enabling requires WIF binding + downstream impersonation switch. PREVIEW (not yet validated against a real apply)."
  type        = bool
  default     = false
}
