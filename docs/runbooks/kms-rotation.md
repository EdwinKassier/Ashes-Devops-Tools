...
```bash
terraform -chdir=envs/gcp-organization plan -target=module.organization.cmek
terraform -chdir=envs/gcp-organization apply -target=module.organization.cmek
```
...
```bash
terraform -chdir=envs/gcp-organization refresh -target=module.organization.cmek
```
