...
| Inspection VPC + Network Firewall + centralized egress | SRA "Network account"; centralized inspection | `modules/aws/network-firewall` + `modules/aws/stages/network-hub` | Stage builds an inspection VPC (Network Firewall endpoints) and an egress VPC; stage-level EIP+NAT per egress public subnet routes through the inspection attachment. `enable_network_firewall` cost-gated (default on). |
