...
| GuardDuty org-wide (auto_enable = ALL, protection plans) | SRA "GuardDuty" delegated admin | `modules/aws/security/guardduty-org` | `auto_enable_organization_members = "ALL"`; detector + protection-plan features per enabled Region; `enable_ebs_malware_protection` (cost-gated, default on). |
| Security Hub CENTRAL configuration (FSBP / CIS / NIST) | SRA "Security Hub" central config | `modules/aws/security/securityhub-org` | CENTRAL configuration policy associated at org root from the Security Tooling delegated admin; standard set defaults to FSBP + CIS 1.4 + NIST 800-53 r5 computed from `home_region`. |
...
| Macie / Inspector / Detective / Resource Explorer | SRA "Additional security services" | `modules/aws/security/org-security-service` | `enabled_services` set gates `macie`, `inspector`, `detective`, `resource-explorer`. Default `["macie","inspector"]`; Detective OFF by default per SRA. |
...
**Gated / optional** (present in the repo but disabled by default, opt-in per environment): `modules/aws/security/edge-security` (`enable_edge`), `modules/aws/data/private-ca` (`enable_private_ca`), `modules/aws/security/secrets-baseline` (`enable_secrets_baseline`), `modules/aws/governance/service-quotas` (`enable_service_quotas`), `modules/aws/network/network-access-analyzer` (`enable_network_access_analyzer`), and Detective within `modules/aws/security/org-security-service` (off by default per SRA). Note also that `network-access-analyzer` manages only the reachability *scope* — scope *analyses* are run out-of-band.
